package com.jishang.bimeng.activity.yuewan.fq;
...
import com.jishang.bimeng.entity.yuewan.fq.Yw_fqDataEntity;
import com.jishang.bimeng.entity.yuewan.fq.Yw_fqEntity;
...
public class Yw_fqActivity extends BaseActivity {
	...
	private List<Yw_fqDataEntity> entities_wfq = new ArrayList<Yw_fqDataEntity>();
	private List<Yw_fqDataEntity> entities_wcjfq = new ArrayList<Yw_fqDataEntity>();
	...
	public int initResource() {
		return R.layout.activity_fq_yuewan;
	}
	...
	public void getWfq() {
		...
		new Thread() {
			public void run() {
				String url = UrlUtils.BASEURL+"v1/yw/my_creat_yw_ing.json";
				String resusult = MyHttpRequest.getHttpPostBasic(url, params,
						token);
				if (resusult != null) {
					Log.e("result_wfq", resusult.toString());
					try {
						Yw_fqEntity entity = mGson.fromJson(resusult,
								Yw_fqEntity.class);
						 Log.e("entity--wfq", entity.toString());
	...
	public void getWcjfq() {
		...
		new Thread() {
			public void run() {
				String url = UrlUtils.BASEURL+"v1/yw/my_creat_yw_past.json";
				String resusult = MyHttpRequest.getHttpPostBasic(url, params,
						token);
				if (resusult != null) {
					Log.e("result--wcjfq", resusult.toString());
					try {
						Yw_fqEntity entity = mGson.fromJson(resusult,
								Yw_fqEntity.class);
						 Log.e("entity_wcjfq", entity.toString());
	...
	public class ItemListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				convertView = View
						.inflate(mContext, R.layout.yzlist_item, null);
			...
			final Yw_fqDataEntity entity = entities_wfq.get(position);
		

			viewHolder.mTv_yz_title.setText(entity.getYw_title());
			viewHolder.mTv_server.setText(entity.getYw_server());
			viewHolder.mTv_peple_money.setText(entity.getItem_money());
			viewHolder.mTv_yz_name.setText("测试名字");
			viewHolder.mTv_start_time.setText(times(entity.getYw_start_time()));
			viewHolder.mTv_location.setText("测试地址");

			imageLoader_head.displayImage(entity.getYw_img(), viewHolder.image,
					options_head);
			viewHolder.mTv_begin.setOnClickListener(new OnClickListener() {

				@Override
				public void onClick(View v) {
					startGame(entity.getYw_id());
	...
	public void startGame(String yz_id){
		params.add(new BasicNameValuePair("yw_id", yz_id));
		new Thread(){
			public void run() {
				String url=UrlUtils.BASEURL+"v1/yw/start_yw.json";
...
//					Log.e("entity", entity.toString());
	...
	public class ItemListAdapter_wcj extends BaseAdapter {
		...
		@Override
		public int getCount() {
			if (entities_wcjfq == null) {
				return 0;
			}
			return entities_wcjfq.size();
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			Yw_fqDataEntity entity = entities_wcjfq.get(position);
